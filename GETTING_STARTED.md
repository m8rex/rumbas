...
- Run `docker run --rm -it -v <absolute_path_to_rumbas-examples_repo>:/rumbas ghcr.io/m8rex/rumbas:0.6.2 <exam_or_question_file>`
  - e.g.`docker run --rm -it -v C:\Users\jesse\Documents\rumbas-examples:/rumbas ghcr.io/m8rex/rumbas:0.6.2 exams/M0/algebra/begintest.yaml`
...
set str=%*
set "str=%str:\=/%"
docker run --rm -v %cd%:/rumbas ghcr.io/m8rex/rumbas:0.6.2 %str%
```
...
```bat
@echo off
docker run -it --rm -v %cd%:/rumbas --entrypoint=sh ghcr.io/m8rex/rumbas:0.6.2
```
...
```sh
#!/bin/sh
docker run --rm -v $PWD:/rumbas ghcr.io/m8rex/rumbas:0.6.2 $@
```
...
```sh
#!/bin/sh
docker run -it --rm -v $PWD:/rumbas --entrypoint=sh ghcr.io/m8rex/rumbas:0.6.2
```
